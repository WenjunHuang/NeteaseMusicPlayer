...
namespace MusicPlayer::ViewModel {
    ...
    void SongCategoryListViewModel::reload() {
        ...
        connect(response,
                &APIResponseHandler<APIPlaylistCatListData>::finished,
                [this](const APIResponse<APIPlaylistCatListData>& reply) {
                    std::visit(
                        [this](auto& value) {
                            if constexpr (std::is_convertible_v<decltype(value), APIPlaylistCatListData>) {
                                ...
                                for (const auto& sub : value.sub) {
                                    catItems.insert(sub.category,
                                                    QVariant::fromValue(
                                                        SongCategoryListViewModelReadyStateCategoryItem{sub.name, sub.hot}));
                                ...
                                std::sort(keys.begin(), keys.end(), [](const auto& first, const auto& second) {
                                    return first < second;
                                });
