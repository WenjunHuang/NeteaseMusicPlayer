...
#include "../MusicPlayer/api/data/dj_banner.h"
        ...
        std::visit(
            [](const auto& v) {
                if constexpr (std::is_convertible_v<
                                  decltype(v),
                                  MusicPlayer::API::APIDJBannersData>) {
                    qDebug() << v.data.size();
                } else {
                    std::visit([](const auto& error) { qDebug() << "error"; },
                               v);
                }
                qApp->quit();
            },
    ...
    observe(f).subscribe([](Response<APIBannersData> value) {
        std::visit(
            [](const auto& v) {
                if constexpr (std::is_convertible_v<
                                  decltype(v),
                                  MusicPlayer::API::APIBannersData>) {
    ...
    observe(f).subscribe([](Response<APIPersonalizedData> value) {
        std::visit(
            [](const auto& v) {
                if constexpr (std::is_convertible_v<
                                  decltype(v),
                                  MusicPlayer::API::APIPersonalizedData>) {
                    qDebug() << v.result.size();
                    for (const auto& item : v.result) {
    ...
    observe(f).subscribe([](Response<APIUserLoginData> value) {
        std::visit(
            [](const auto& v) {
                if constexpr (std::is_convertible_v<
                                  decltype(v),
                                  MusicPlayer::API::APIUserLoginData>) {
                    if (v.cookieToken)
...
TEST_CASE("userPrivateMessages", "[MusicAPI]") {
    ...
    auto f = api.userPrivateMessages(
        "; "
        "MUSIC_U="
        "32506ca1e2f682f81fb7fe6a0ebf3c746125db7247d6651fd053861143761ee3053a3c"
        "2009db5a182cd526183697c5bcfe2897047e8106fb; "
        "__csrf=0b15c1aee3089531a9e5575b5053c104");
    ...
    observe(f).subscribe([](Response<APIPlaylistCatListData> value) {
        std::visit(
            [](const auto& v) {
                if constexpr (std::is_convertible_v<
                                  decltype(v),
                                  MusicPlayer::API::APIPlaylistCatListData>) {
                    qDebug() << v.all.name;
                    qDebug() << v.sub.size();
                } else {
                    std::visit([](const auto& error) { qDebug() << "error"; },
