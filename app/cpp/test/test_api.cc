...
TEST_CASE("djBanner", "[MusicAPI]") {
    ...
    MusicAPI api;
    auto f = api.djBanner();
    QObject::connect(f,&APIResponseHandler<APIDJBannersData>::finished,[](const APIResponse<APIDJBannersData>& value) {
        ...
        std::visit(
            [](const auto& v) {
                ...
                } else if constexpr(std::is_convertible_v<decltype(v),APIError>) {
                    qDebug() << "error";
                    std::visit(overload{[](const ErrorResponseError& error) {
                        qDebug() << "ErrorResponseError(" << error.code << "," << error.message.value_or("");
                        },
                                        [](const NetworkError& error) {
                        qDebug() << "NetworkError:" << error.message;
                        },
                                        [](const JsonFormatError& error) {
                        qDebug() << "JsonFormatError";
                    }},
                               v);
                }
                qApp->quit();
...
//    std::move(f).via(mainExecutor()).thenValue([](APIResponse<APIDJCategoryExcludeHotData> value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.banner().via(mainExecutor()).thenValue([](const APIResponse<APIBannersData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.personalizedNewSong().via(mainExecutor()).thenValue([](const APIResponse<APIPersonalizedNewSongData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.djCategoryRecommend().via(mainExecutor()).thenValue([](const APIResponse<APIDJCategoryRecommendData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.personalized().via(mainExecutor()).thenValue([](const APIResponse<APIPersonalizedData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.topPlaylistHighQuality("全部", 1).via(mainExecutor()).thenValue([](const APIResponse<APITopPlayListData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.playlistDetail(1997190595).via(mainExecutor()).thenValue([](const APIResponse<APIPlayListDetailData>& value) {
//        std::visit(
//            [](const auto& v) {
...
////    observe(f).subscribe([](APIResponse<APIUserLoginData> value) {
...
////    __csrf=0b15c1aee3089531a9e5575b5053c104"); observe(f).subscribe([](APIResponse<APIUserPrivateMessagesData> value) {
////      std::visit([](const auto &v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.playlistCatlist().via(mainExecutor()).thenValue([](const APIResponse<APIPlaylistCatListData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.songDetail(QVector{347230, 347231}).via(mainExecutor()).thenValue([](const APIResponse<QString>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.songUrl(347230).via(mainExecutor()).thenValue([](const APIResponse<APISongUrlData>& value) {
//        std::visit(
//            [](const auto& v) {
...
//    auto app = setUp();
//    MusicAPI api;
//    api.artistDesc(6452).via(mainExecutor()).thenValue([](const APIResponse<QString>& value) {
//        std::visit(
//            [](const auto& v) {
//                if constexpr (std::is_convertible_v<decltype(v), QString>) {
//                    qDebug() << v;
